...
export default class BaseCosmosAssetScanner extends BaseAssetScanner {
	...
	async _init() {
		await super._init()
		this.rpcClient = new HttpClient(this.config.endpoint)
		this.tmClient = await this.rateLimiter.exec(() => Tendermint34Client.connect(this.config.endpoint))
		this.client = await this.rateLimiter.exec(() => StargateClient.connect(this.config.endpoint))
		this.cwClient = await this.rateLimiter.exec(() => CosmWasmClient.connect(this.config.endpoint))
		// TODO: promise all
	}
