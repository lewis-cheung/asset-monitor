...
export default class BaseCosmosAssetScanner extends BaseAssetScanner {
	...
	/** @type {string[]} */							static requiredParamKeys = [ 'endpoint' ]
	/** @type {Map<types.AssetCode, string>} */		tokenInfoByAddr = new Map()
	...
	async _init() {
		await super._init()
		this.rpcClient = new HttpClient(this.paramDict.endpoint)
		this.tmClient = await this.rateLimiter.exec(() => Tendermint34Client.connect(this.paramDict.endpoint))
		this.client = await this.rateLimiter.exec(() => StargateClient.connect(this.paramDict.endpoint))
		this.cwClient = await this.rateLimiter.exec(() => CosmWasmClient.connect(this.paramDict.endpoint))
		// TODO: promise all
	}
