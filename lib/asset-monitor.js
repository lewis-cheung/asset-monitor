...
export default class AssetMonitor {
		...
		while (trial <= 5) {
			...
			try {
				const promises = assetQueries.map(query => new Promise(async (resolve, reject) => {
					const chainIdentifier = AssetScannerConfig.getChainIdentifier(query.type, query.chain)
					const scannerConfigs = assetScannerConfigsMap.get(chainIdentifier)
					if (!scannerConfigs || scannerConfigs.length == 0) reject(`No scanner found for query: ${JSON.stringify(query)}`)
	...
	async getAssetScannerConfigsMap() {
		...
		for (const config of assetScannerConfigs) {
			const key = config.chainIdentifier
